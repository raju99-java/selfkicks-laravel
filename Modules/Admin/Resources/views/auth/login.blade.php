...
<!-- END LOGIN -->
<!-- BEGIN COPYRIGHT -->
<div class="copyright" style="color: #000;"> {{date('Y')}} &copy; {{env('APP_NAME', 'Self Kicks')}} - Admin Login. </div>

<script>
    $(document).ready(function () {
        $.backstretch([
            "{{ URL::asset('public/backend/background/1.webp') }}",
            "{{ URL::asset('public/backend/background/2.webp') }}",
            "{{ URL::asset('public/backend/background/net.jpg') }}",
        ], {
            fade: 1000,
            duration: 8000
        }
