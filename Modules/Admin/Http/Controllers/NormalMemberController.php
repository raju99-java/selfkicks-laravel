...
class NormalMemberController extends AdminController {
    ...
    public function get_user_list() {
        ...
        return view('admin::normal-member.list');
    }

    public function get_user_list_datatable() {
        $user_list = UserMaster::where('type_id','<>', '1')->where('subcription_id', '=', '1')->where('status', '<>', '3')->orderBy('id','desc')->get();
        return Datatables::of($user_list)
                        ...
                        ->addColumn('action', function ($model) {
                            return
                            '<a href="' . Route("normal-member-edit", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-edit"></i> Edit</a>' .
                            '<a href="javascript:;" onclick="deleteNormalMember(this);" data-href="' . Route("normal-member-delete", [$model->id]) . '" class="btn btn-xs btn-primary pull-left"><i class="fa fa-trash"></i> Delete</a>';
                        })
    ...
    public function get_edit_user($id) {
        ...
        $data['id'] = $id;
        return view('admin::normal-member.edit', $data);
    }
    ...
    public function post_edit_user(Request $request, $id) {
        ...
        if ($validator->passes()) {
            ...
            $model->update($input);
            return redirect()->route('normal-member')->with('success_msg', 'Member updated successfully.');
        } else {
            return redirect()->route('normal-member')->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
    }
