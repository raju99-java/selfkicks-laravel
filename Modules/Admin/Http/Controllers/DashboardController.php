...
class DashboardController extends AdminController {
    ...
    public function index() {
        ...
        $data_msg['total_plan_hist'] = SubscriptionHistory::count();
        $data_msg['total_kyc_req'] = UserMaster::where('kyc_verified', '1')->count();
        $data_msg['total_slider'] = Slider::where('status', '1')->count();
        $data_msg['total_plan'] = Subscriptions::where('status', '1')->count();
        $data_msg['total_user'] = UserMaster::where('type_id','<>', '1')->where('status','<>', '3')->where('subcription_id','=', '0')->count();
        $data_msg['total_member'] = UserMaster::where('type_id','<>', '1')->where('status','<>', '3')->where('subcription_id','=', '1')->count();
        $data_msg['total_premium_member'] = UserMaster::where('type_id','<>', '1')->where('status','<>', '3')->where('subcription_id','=', '2')->count();
        $data_msg['total_video'] = Video::count();
        $data_msg['total_enq'] = Contactus::count();
        $data_msg['total_prime_video'] = Video::where('prime', '1')->count();
        $data_msg['total_earn'] = EarnPoints::count();
