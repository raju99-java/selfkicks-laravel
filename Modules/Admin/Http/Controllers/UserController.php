...
class UserController extends AdminController {
    ...
    public function get_user_list_datatable() {
        $user_list = UserMaster::where('type_id','<>', '1')->where('subcription_id', '=', '0')->where('status', '<>', '3')->orderBy('id','desc')->get();
        return Datatables::of($user_list)
                        ...
                        ->rawColumns(['image','subcription_id','status','created_at', 'action'])
                        ->make(true);
    }
    ...
    public function post_edit_user(Request $request, $id) {
        ...
        if ($validator->passes()) {
            ...
            $model->update($input);
            return redirect()->route('user')->with('success_msg', 'User updated successfully.');
        } else {
            return redirect()->route('user')->withErrors($validator)->withInput()->with('error_msg', 'Something went wrong please check your input.');
        }
    }
